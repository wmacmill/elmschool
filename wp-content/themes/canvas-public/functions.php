...
//change the url for the image above the login section on login page
...
//update the painter user meta after enrolling in the painter site
add_action( 'gform_user_registered', 'will_add_custom_user_meta', 10, 4 );

function will_add_custom_user_meta( $user_id, $feed, $entry, $user_pass ) {
    ...
    if ( $country == 'Canada' ) {
        $group = 18;
        $state_group_id = rgar ($entry, '10');
        $state = rgar ($entry, '10');
    }
    elseif ( $country == 'USA') {
        $group = 22;
        $state_group_id = rgar ($entry, '11');
        $division_group_id = wp_get_post_parent_id ($state_group_id); //get the parent post id to use as division id
        update_user_meta ( $user_id, 'learndash_group_users_' . $division_group_id, $division_group_id);//enroll in division group
    }

    update_user_meta( $user_id, 'learndash_group_users_' . $group , $group ); //enroll in country group
    update_user_meta( $user_id, 'learndash_group_users_' . $state_group_id, $state_group_id);//enroll in state group
    update_user_meta( $user_id, 'Country', $country );//add country meta
    update_user_meta( $user_id, 'State', $state );//add state meta
}


/*
function add_max_to_all_groups () {//this is to add max to all the groups on the painter site as leader. keep disabled when you don't need it as it kills the public site for some reason.
     if ( get_current_blog_id() != 3 ) {
        exit;
    }
    ...
    foreach ( $group_posts as $group) {
        $group_id = $group->ID;
        update_user_meta (20, 'learndash_group_leaders_' . $group_id, $group_id);
    }

}

add_action ( 'init', 'add_max_to_all_groups');*/

...
class Ld_Bulk_Rename{

	public function ldbr_rename_submenu_page() {
		add_submenu_page( 
			...
			array( $this, "ldbr_page_callback" ) );
	}

	public function ldbr_add_submenu( $submenu ) {
	...
	public function ldbr_page_callback(){
		$args = array(
			...
			<div>
					<?php
						if( !empty( $selected_course ) ){
							...
							$this->ldbr_display_renaming( $selected_course, get_the_title( $selected_course ) );

							foreach ($lesson_ids as $id => $title) {
								$this->ldbr_display_renaming( $id, $title );
							}
	...
	public function ldbr_display_renaming( $post_id, $title ){
	...
	public function ldbr_bulk_rename_callback(){
		...
		if( wp_verify_nonce( $security, 'bulk_renaming' ) ) {
			...
			foreach( $rename_data as $post_id => $new_title ) {
				if( get_the_title( $post_id ) != trim( $new_title ) ){
					$this->update_the_post( $post_id, $new_title );
				}
			}
	...
	public function update_the_post( $post_id, $new_title ){
		$post_arr = array( 
						'ID'        => $post_id,
